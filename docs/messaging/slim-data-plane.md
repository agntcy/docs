...
This approach enables efficient client discovery in fact the message will be deliver by the SLIM network to a client that is able
to process it, even if the real name of the client is unknow. The anycast forwarding is mostily used in this discovery phase.
