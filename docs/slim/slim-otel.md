...
The SLIM OpenTelemetry [repository](https://github.com/agntcy/slim-otel) provides custom receiver and exporter
...
and logs) using SLIM's end-to-end encryption with MLS (Message Layer Security)
...
4. Automatically detects the signal type (traces, metrics, or logs) by inspecting
   the received data
5. Routes the telemetry data to the appropriate consumer and passes it through
   the configured processors and exporters in the collector pipeline
6. Supports multiple concurrent sessions from different senders simultaneously

### Signal Routing

Each signal type is routed to channels configured for that specific signal type:

- **Traces** → channels with `signal: traces`
- **Metrics** → channels with `signal: metrics`
- **Logs** → channels with `signal: logs`

This separation allows for fine-grained control over which participants receive
which types of telemetry data.
...
- [Go](https://go.dev/doc/install) - For building the collector and test
  applications
- [Task](https://taskfile.dev/installation/) - Task runner for executing build
  and run commands
...
!!! note "Using SLIM Components in Your Own Collector"
...
In this tutorial, we'll set up two collectors:

- **Sender Collector**: Uses the SLIM exporter to send telemetry data over SLIM channels
- **Receiver Collector**: Uses the SLIM receiver to accept telemetry data from SLIM channels
...
**SLIM Exporter Configuration Options:**
...
**SLIM Receiver Configuration Options:**
...
Install [telemetrygen](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/cmd/telemetrygen):
