...
Migration between profiles is possible in both directions. The following procedures cover migrating from `https_spiffe` to `https_web` and from `https_web` to `https_spiffe`.

### Migration from https_spiffe to https_web
...
### Migration from https_web to https_spiffe

**Prerequisites:**

- SSL passthrough enabled on NGINX ingress controller.
- Bootstrap bundles exchanged with all federation partners. See [Bootstrap Bundle Exchange Process](#bootstrap-bundle-exchange-process-https_spiffe) for detailed procedures.
...
2. Exchange bootstrap bundles with federation partners:
    ...
    - Extract your bundle (see [Bootstrap Bundle Exchange Process](#bootstrap-bundle-exchange-process-https_spiffe#setting-up-bootstrap-bundles)).
    - Exchange bundles with federation partners using your organization's secure procedures.
    - Validate received bundles.
