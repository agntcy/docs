...
For step-by-step federation setup with the public Directory network, see [Connect to the Public Directory](federation.md).
...
flowchart TB
  Remote[Remote SPIRE Bundle Consumer]
  NGINX[NGINX Ingress Controller<br/>• SSL Passthrough enabled<br/>• Forwards encrypted traffic opaque<br/>• No inspection or termination]
...
### Step 1: Extract Bundle from SPIRE Server

#### Method A: Using kubectl exec (Recommended)
...
#### Method B: Using spire-server CLI Locally
...
#### Method C: Via Federation Endpoint (After Initial Setup)
...
### Step 3: Configure Federation with Bundle
...
**Note:** Organizations must establish their own secure procedures for exchanging bootstrap bundles with federation partners. The bundle exchange mechanism (email, file transfer, version control, etc.) should align with organizational security policies.

---

For step-by-step configuration of each profile, see [Federation Profile Configuration](federation-profile-configuration.md).
...
Migration from https_web to https_spiffe is straightforward if requirements change.
...
**Resolution:**
```yaml
# Ensure tlsSecret is specified in ingress configuration
ingress:
...
## Profile Migration Procedures

### Migration: https_spiffe to https_web

**Prerequisites:**
- cert-manager installed in cluster
- ClusterIssuer configured for certificate issuance
- DNS configured for federation endpoint
...
3. Update SPIRE server configuration:
   ...
   global:
     spire:
       ingressControllerType: other  # Change from: ingress-nginx
   ...
   ingress:
     tlsSecret: "spire-server-federation-cert"
     annotations:
       nginx.ingress.kubernetes.io/ssl-passthrough: "false"  # Change from: "true"
       nginx.ingress.kubernetes.io/backend-protocol: "HTTPS"  # Add this line
       nginx.ingress.kubernetes.io/proxy-ssl-server-name: "on"  # Add this line
       nginx.ingress.kubernetes.io/proxy-ssl-name: "spire.example.com"  # Add this line
   ```

4. Update all federation files:
   ```yaml
   # Remove these fields:
   ...
   # Update profile type:
   bundleEndpointProfile:
     type: https_web  # Change from: https_spiffe
   ```

5. Deploy changes via GitOps/Helm

6. Monitor certificate issuance:
   ```bash
   kubectl get certificate -n dir-spire --watch
   ```

7. Verify bundle endpoint:
   ```bash
   curl https://spire.example.com
   ```

8. Monitor bundle refresh in logs:
   ```bash
   kubectl logs -n dir-spire -l app.kubernetes.io/name=server -c spire-server --follow | grep "Bundle refreshed"
   ```

**Expected Downtime:** Approximately 5 minutes during certificate issuance

---

### Migration: https_web to https_spiffe

**Prerequisites:**
- SSL passthrough enabled on NGINX ingress controller
- Bootstrap bundles exchanged with all federation partners (see "Bootstrap Bundle Exchange Process" section)

**Migration Steps:**

1. Enable SSL passthrough on NGINX:
   ```bash
   kubectl patch deployment -n ingress-nginx ingress-nginx-controller --type='json' \
     -p='[{"op": "add", "path": "/spec/template/spec/containers/0/args/-", "value":"--enable-ssl-passthrough"}]'
   ```

2. Exchange bootstrap bundles with federation partners:
   - Extract your bundle (see Step 1 in "Bootstrap Bundle Exchange Process")
   - Exchange bundles with federation partners using your organization's secure procedures
   - Validate received bundles (see Step 2 in "Bootstrap Bundle Exchange Process")
...
3. Update SPIRE server configuration:
   ...
   global:
     spire:
       ingressControllerType: ingress-nginx  # Change from: other
   ...
   ingress:
     ...
     # tlsSecret: "spire-server-federation-cert"
     annotations:
       nginx.ingress.kubernetes.io/ssl-passthrough: "true"  # Change from: "false"
       # Remove these lines:
...
4. Update all federation files:
   ...
   # Add these fields:
   bundleEndpointProfile:
     type: https_spiffe  # Change from: https_web
     endpointSPIFFEID: spiffe://example.com/spire/server
   trustDomainBundle: |-
     {
       "keys": [...]
...
6. Verify SSL passthrough and bundle refresh:
   ```bash
   # Verify SPIFFE certificate with URI SAN
   openssl s_client -connect spire.example.com:443 -showcerts 2>&1 | grep "URI:spiffe://"
   
   # Monitor bundle refresh in logs
   kubectl logs -n dir-spire -l app.kubernetes.io/name=server -c spire-server --follow | grep "Bundle refreshed"
   ```

**Expected Downtime:** Approximately 2 minutes during SPIRE server pod restart

---

## Operational Best Practices

### Universal Best Practices (Both Profiles)

1. **Consistent className Configuration**
...
2. **Enable ClusterFederatedTrustDomain Controller**
...
3. **Implement Bundle Refresh Monitoring**
...
1. **Use Production Certificate Authority**
...
3. **Validate External Network Accessibility**
...
1. **Maintain Bootstrap Bundle Freshness**
...
2. **Document Bundle Exchange Procedures**
...
3. **Validate SSL Passthrough Persistence**
